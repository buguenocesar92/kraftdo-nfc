{{-- Token Gift Advanced Modal Component --}}
...
<div class="fixed inset-0 bg-black z-50" 
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4 overflow-hidden">
        <!-- Loading State -->
        <div x-show="currentImage.loading" 
        ...
        <img class="max-w-none max-h-none object-contain transition-transform duration-200 select-none" 
             ...
             x-show="!currentImage.loading && !currentImage.error"
             draggable="false">
    </div>
    ...
    <div class="absolute top-0 left-0 right-0 bg-gradient-to-b from-black to-transparent p-4 z-30">
        <div class="flex items-center justify-between">
            ...
            <div class="text-white flex-1">
                <h3 class="text-lg font-semibold" x-text="currentImage.alt || 'Imagen de galería'"></h3>
                <p class="text-sm text-gray-300" x-show="currentImage.caption" x-text="currentImage.caption"></p>
            </div>
    ...
    <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4 z-30">
        ...
        <!-- Thumbnails -->
        <template x-if="galleryImages.length > 1 && galleryImages.length <= 10">
            <div class="flex justify-center mt-4 space-x-2 overflow-x-auto pb-2">
                <template x-for="(image, index) in galleryImages" :key="index">
                    <button x-on:click="currentImageIndex = index; updateCurrentImage(); preloadAdjacentImages()"
                            class="relative flex-shrink-0 w-12 h-12 rounded-lg overflow-hidden border-2 transition-all"
                            :class="currentImageIndex === index ? 'border-white scale-110' : 'border-transparent opacity-60 hover:opacity-100'">
