...
<script>
    ...
    // Alpine.js component for smart contact saving
    window.contactComponent = function() {
        return {
            // Main function: Smart contact save with hierarchical fallbacks
            async saveContact() {
                ...
                try {
                    ...
                    // Method 1: Web Share API (primary method for iOS/Android)
                    if (navigator.share && navigator.canShare) {
                        try {
                            const file = new File([vcard], `${this.sanitizeFileName(contactInfo.name)}.vcf`, {
                                type: 'text/vcard'
                            });
                            ...
                            if (navigator.canShare({ files: [file] })) {
                                await navigator.share({
                                    ...
                                    text: `Guarda el contacto de ${contactInfo.name}`
                                });
                                success = true;
                            }
                        } catch (error) {
                    ...
                    // Method 2: Android Intent URL fallback
                    if (!success && this.isAndroid()) {
                        success = this.saveContactAndroid();
                    }
                    
                    // Method 3: Final fallback - vCard download
                    if (!success) {
                        success = this.downloadVCard();
                    }
            ...
            downloadVCard() {
                try {
                    ...
                    link.href = url;
                    link.download = `${this.sanitizeFileName(contactInfo.name)}.vcf`;
    ...
    async function smartSaveContact() {
        const component = window.contactComponent();
