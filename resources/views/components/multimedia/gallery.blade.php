{{-- Generic Multimedia Gallery Component --}}
@props([
    'images' => [],
    'theme' => [
...
@if($images && count($images) > 0)
    ...
    <div class="bg-gradient-to-br {{ $theme['background'] }} rounded-xl p-6" x-data="multimediaGallery({
        images: [
        ...
        @if($layout === 'masonry')
            <div class="columns-1 {{ $columns }} {{ $gap }} gallery-masonry w-screen left-1/2 -translate-x-1/2 sm:left-0 sm:translate-x-0 sm:w-full relative">
        @else
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 {{ $gap }}">
        @endif
            @foreach($images as $index => $image)
                <div class="relative group cursor-pointer gallery-item mb-1 sm:mb-4" 
                     x-on:click="openModal(images[{{ $index }}], {{ $index }})">
                    
                    <!-- Skeleton Loader -->
                    <div class="bg-gray-200 rounded-lg animate-pulse aspect-[4/3] z-10"
                         x-show="!imageLoaded[{{ $index }}] && !imageError[{{ $index }}]">
        ...
        <!-- Gallery Stats -->
        @if($showStats)
            <div class="mt-4 text-center text-sm {{ $theme['text'] }}">
        ...
        <!-- Modal Component (within gallery context) -->
        <x-multimedia.modal />
    </div>
@else
    <!-- Empty State -->
